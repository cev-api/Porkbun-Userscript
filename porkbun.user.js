...
// @version      3.1
// @description  Filters Porkbun results, supports TLD filters, inline renewal prices, displayed currency conversion, timeout counting, and auto-expands all extensions.
// @author       CevAPI
...
(function()
{
    ...
    const STATS_ID = 'pb-filter-stats';
    ...
    const PRICE_RENDER_ATTR = 'data-pb-rendered-price';
    const FX_CACHE_PREFIX = 'pb_fx_rate_';
    const FX_CACHE_TTL_MS = 1000 * 60 * 60 * 12;
    const FX_CACHE_STALE_TTL_MS = 1000 * 60 * 60 * 24 * 7;
    ...
    function hasTimedOutText(element)
    {
        if(!element) return false;
        ...
        if(text.includes('timedout')) return true;
    ...
    function mainRowShouldHide(row)
    {
        ...
        if(hasTimedOutText(row)) return true;
    ...
    function sidebarCardShouldHide(card)
    {
        ...
        if(text.includes('unavailable')) return true;
        if(text.includes('sold')) return true;
        if(hasTimedOutText(card)) return true;
    ...
    function captureBasePriceHtml()
    {
        getPriceContainers().forEach(container =>
        {
            ...
            if(container.getAttribute(PRICE_RENDER_ATTR) === '1') return;
    ...
    function restoreBasePriceHtml()
    {
        getPriceContainers().forEach(container =>
        {
            ...
            if(container.innerHTML !== container.dataset.pbBaseHtml)
            {
                container.innerHTML = container.dataset.pbBaseHtml;
            }

            container.removeAttribute(PRICE_RENDER_ATTR);
        });
    }
    ...
    function getCachedFxRate(targetCurrency, allowStale)
    {
        try
        {
            ...
            const maxAge = allowStale ? FX_CACHE_STALE_TTL_MS : FX_CACHE_TTL_MS;
            if(Date.now() - parsed.timestamp > maxAge) return null;

            return {
                rate: parsed.rate,
                stale: Date.now() - parsed.timestamp > FX_CACHE_TTL_MS
            };
        }
        catch(error)
        {
    ...
    {
        const response = await fetch(`https://api.frankfurter.app/latest?from=USD&to=${encodeURIComponent(targetCurrency)}`, {
            method: 'GET',
            credentials: 'omit',
            cache: 'no-store'
        });
        ...
        {
            throw new Error('Frankfurter no rate');
        }

        return data.rates[targetCurrency];
    }

    async function fetchOpenExchangeRate(targetCurrency)
    {
        ...
        if(!data || !data.rates || typeof data.rates[targetCurrency] !== 'number')
        {
            throw new Error('Fallback no rate');
        }
    ...
    async function fetchFxRate(targetCurrency)
    {
        ...
        const cached = getCachedFxRate(targetCurrency, false);
        ...
        {
            const rate = await fetchOpenExchangeRate(targetCurrency);
            setCachedFxRate(targetCurrency, rate);
            return rate;
        }
    }
    ...
    function ensureOverlay()
    {
        ...
        overlay.innerHTML = `
            ...
            <div id="${BODY_ID}" class="pb-tld-filter-body">
                ...
                <div id="${STATS_ID}">Timed out hidden: 0</div>
            </div>
        `;
    ...
    function updateStats(timedOutHiddenCount)
    {
        const stats = document.getElementById(STATS_ID);
        if(!stats) return;

        stats.textContent = 'Timed out hidden: ' + timedOutHiddenCount;
    }

    function filterMainRows()
    {
        let timedOutHiddenCount = 0;

        getMainRows().forEach(row =>
        {
            const timedOut = hasTimedOutText(row);
            const shouldHide =
    ...
    function filterSidebarCards()
    {
        ...
        {
            const timedOut = hasTimedOutText(card);
            const shouldHide =
    ...
    async function getConversionRate()
    {
        ...
        try
        {
            const rate = await fetchFxRate(targetCurrency);
        ...
        {
            const staleCached = getCachedFxRate(targetCurrency, true);

            if(staleCached !== null)
            {
                if(requestToken === fxRequestToken)
                {
                    setFxStatus('Showing cached ' + targetCurrency + ' prices');
                }

                return {
                    currency: targetCurrency,
                    rate: staleCached.rate,
                    converted: true
                };
            }
            ...
            if(requestToken === fxRequestToken)
            {
                setFxStatus('Conversion failed, showing USD');
            }
    ...
    function getPriceRenderKey(currency, rate, inlinePricesEnabled, convertPricesEnabled)
    {
        return [
            currency,
            String(rate),
            inlinePricesEnabled ? 'inline' : 'stacked',
            convertPricesEnabled ? 'converted' : 'usd'
        ].join('|');
    }
    ...
    async function rebuildPrices()
    {
        ...
        const renderKey = getPriceRenderKey(
            conversion.currency,
            conversion.rate,
            inlinePricesEnabled,
            convertPricesEnabled
        );

        if(renderKey === lastRenderedPriceKey)
        {
            return;
        }
        ...
        try
        {
            getPriceContainers().forEach(container =>
            {
                ...
                scratch.innerHTML = container.dataset.pbBaseHtml;

                const model = extractPriceModel(scratch);
                if(!model) return;

                const renderedHtml = buildRenderedPriceHtml(
                    model,
                    conversion.currency,
                    conversion.rate,
                    inlinePricesEnabled
                ...
                container.setAttribute(PRICE_RENDER_ATTR, '1');
            });

            lastRenderedPriceKey = renderKey;
        }
        finally
        {
            observerPaused = false;
    ...
    function refreshNow()
    {
        ...
        Promise.resolve().then(async () =>
        {
            try
            {
                ...
                const mainTimedOutHiddenCount = filterMainRows();
                const sidebarTimedOutHiddenCount = filterSidebarCards();

                cleanupExplicitEmptyRows();
                updateStats(mainTimedOutHiddenCount + sidebarTimedOutHiddenCount);
